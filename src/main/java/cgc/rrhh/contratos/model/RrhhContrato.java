...
"INNER JOIN RRHH_DEPARTAMENTO D ON M.DEPARTAMENTO = D.DEPARTAMENTO " +
"LEFT JOIN RRHH_COLEGIO_PROFESIONAL CP ON A.COLEGIO_PROFESIONAL = CP.COLEGIO_PROFESIONAL " +
"INNER JOIN ( SELECT C.ID_CONTRATO, C.ID_CATALOGO_ESTADO FROM RRHH_CONTRATO_ESTADO C WHERE ESTADO = 'A' AND C.ID_CATALOGO_ESTADO = NVL(?3,C.ID_CATALOGO_ESTADO)) CE ON C.ID_CONTRATO = CE.ID_CONTRATO " +
"WHERE L.ESTADO IN ('I', 'A') " +
"AND C.ANIO = ?1 " +
"AND L.RENGLON = NVL(?2,L.RENGLON) "+                                
"ORDER BY CE.ID_CATALOGO_ESTADO,L.FECHA_INSERT ASC ",
                        resultSetMapping = "ResultsContrato"),
